...
Extend `.claude/hooks/session-stop.sh` with a **primary decision step** placed *before* the spec-023 porcelain-compare:

1. If `edited-files.txt` exists and (a) is empty OR (b) every listed path is clean in `git status --porcelain` (committed or reverted) → `exit 0` silently.
2. If `edited-files.txt` exists and at least one listed path is still dirty → fall through to the existing block-unless-SESSION-updated logic.
3. If `edited-files.txt` does NOT exist (legacy sessions started before this spec landed, hook disabled via `CLAUDE_SKIP_SESSION_HOOKS=1`, or a fork that hasn't synced 030 yet) → fall through to spec 023 unchanged.
...
- **Hook latency on hot edit loops.** PostToolUse runs synchronously. A delegated sub-agent doing many small `Edit` calls would compound any per-hook cost. Target: ≤ 5 ms per invocation (jq parse + file lock + line append). Acceptable headroom against the existing PostToolUse hook stack (`post-edit-validate.sh` alone runs the project validator and is the dominant cost; tracker is noise next to that). Verify with `time` once implemented.
