...
**4 dogfood-driven fixes Agent0-side 2026-05-12 (shrnk-mono session `dogfood-mono`).** All landed via real CC session in /home/goat/shrnk-mono surfacing real bugs:
- `7d7bde7 fix(harness-sync)` — `.claude/presence/` was missing from sync manifest; `settings.json` referenced `statusline.mjs` but file didn't propagate → silent statusline absence in forks. Manifest gained `.claude/presence|*.mjs`; gotcha doc added (cross-check settings.json refs against manifest when adding new `.claude/` subdirs).
- `c8f145a fix(runtime-introspect)` — bun's ANSI-colored test output prefixed `^0 fail$` regex; inference fell through to weak `pass/ok` heuristic. `runtime-capture.sh` now strips ANSI sequences (`\x1b\[[0-9;]*[a-zA-Z]`) before storage AND inference. Test 16 + gotcha doc; suite 16/16.
- `90c91b5 fix(validator)` — typecheck-advisory (manifest-as-intent mirroring spec 013 lint). bun/pnpm: tsconfig.json → direct tsc; else scripts.typecheck → script; else omit + advisory. npm: conservative script-only path. Replaces hard-failure pattern that broke validators on early-stage forks. New rule `.claude/rules/typecheck-advisory.md` + 5 tests.
- `dcddf72 fix(validator)` — defensive grep filter against fork-misconfigured .gitignore. `git ls-files --others --exclude-standard` was dumping 15,711 untracked node_modules paths into TDD-warning per-file shell loop, hanging validator > 60s in shrnk-mono. Filter strips node_modules/.venv/__pycache__/target/dist/build/coverage/.next/ etc. before loop. Test 06 (1500 noise files complete <1s); gotcha doc on stack-agnostic .gitignore template.
...
- **Validator hang root cause = TDD warning loop O(untracked files), not bun test.** shrnk-mono dogfood 2026-05-12: `bun test` itself completes in 110ms, but `git ls-files --others --exclude-standard | shell-loop-per-file` over 15k node_modules entries hangs > 60s. Pattern: when a validator hook hangs in a fork, audit `git ls-files --others --exclude-standard | wc -l` first — anything > 1000 means .gitignore is misconfigured and the per-file loop is the bottleneck, not the actual test/build command.
- **Agent0 .gitignore is intentionally stack-agnostic.** Template ships ALL stack-specific patterns (`node_modules/`, `.venv/`, `target/`, `__pycache__/`, etc.) commented out. Forks MUST uncomment per-stack post-clone. Forgetting this propagates as silent `git ls-files` bloat that breaks downstream tools (validator hang, supply-chain audit noise, slow status). Audit command: `git -C <fork> ls-files --others --exclude-standard | awk -F/ '{print $1}' | sort | uniq -c | sort -rn | head` — top entries with high counts are the gaps.
- **`/tmp` tmpfs filling silently breaks the Bash tool.** `bash -x .claude/validators/run.sh` against a multi-workspace fork produced 7.8GB of trace output in `/tmp/v.trace`; subsequent Bash calls returned "exit 1" with NO stderr because stderr write itself failed (`pwd: write error: No space left on device`). Diagnosis: `df -h /tmp` shows 100% use; cleanup with `rm /tmp/<huge-trace-file>`. Avoid `bash -x` on validators; use targeted `bash -v` or insert `echo` markers instead. Same caveat applies to any `tee /tmp/...` from a long-running process.
