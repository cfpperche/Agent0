...
You are running the weekly Claude Code platform audit against `.agent0/memory/cc-platform-hooks.md`.
...
2. Compare the hook events / payload shapes / event semantics list against what `.agent0/memory/cc-platform-hooks.md` documents (especially the 29-event table and payload-shape gotchas).
...
5. If drift exists: apply the proposed edits to `.agent0/memory/cc-platform-hooks.md` (DO NOT commit — leave diff for human review via `git diff`).
