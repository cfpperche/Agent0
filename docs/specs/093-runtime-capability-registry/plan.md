...
- `.claude/harness-sync-baseline.json` — refreshed by the post-implementation sync-harness run that records the new managed-block content + the new rule file in the baseline. This is the only file that changes mechanically rather than by hand-edit.
...
- **Managed-block resync risk for forks.** A fork that has edited the AGENTS.md or CLAUDE.md managed block in-place (instead of using `AGENTS.override.md` per spec 090) will see `sync-harness.sh` flag the new `## Runtime capabilities` paragraph as a customization-refused merge. This is the standard managed-block contract — forks must use the override mechanism — and the post-implementation sync-harness run on this repo refreshes the baseline cleanly because Agent0 itself owns the managed block. Forks see the normal sync-refusal behavior; the upstream change is not a fork-breaking event.
