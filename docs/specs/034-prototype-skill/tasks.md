...
- [x] 29. **Generated monorepos typecheck + lint** — DEFERRED: sandbox skipped the dep-resolution step intentionally; static TS source compiles cleanly as inspection; full run on REMINDERS.md (spec.md plain bullet "Two-stack dogfood verified — both pass typecheck + lint")

## Notes

_Anything that came up during execution that doesn't belong in plan.md but is useful for the PR description or future readers._

- **Spec status: in-progress, not shipped.** 7 of 13 acceptance items (5 scenarios + 5 plain bullets) fully passed; 3 scenarios are PARTIAL (dogfood A bounded to 3/23 routes; 2-stack proven at scaffold level only; idempotency mechanism present but not empirically retried); 1 plain bullet DEFERRED (typecheck+lint on generated monorepos pending fresh-session full E2E run). REMINDERS.md item tracks the deferred E2E. **Status flipping to `shipped` requires the deferred E2E run to verify the typecheck/lint gate per spec acceptance.**
