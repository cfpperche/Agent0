#!/usr/bin/env bash
# Scenario: project memory CONTENT does NOT propagate to forks,
...
#   (c) After sync: fork has NO MEMORY.md and NO foo.md (content stays upstream)
...
---
name: foo
description: Upstream-only memory content that should NEVER ship to forks
metadata:
  type: project
---
foo body
...
- [Foo](foo.md) — Upstream-internal entry that should NOT appear in fork
EOF
...
if [ -f "$FORK/.claude/memory/foo.md" ]; then
  printf 'FAIL: upstream content file foo.md leaked to fork\n'
  exit 1
fi
if [ -f "$FORK/.claude/memory/MEMORY.md" ]; then
  printf 'FAIL: upstream MEMORY.md leaked to fork (each fork must have its own)\n'
  exit 1
fi
