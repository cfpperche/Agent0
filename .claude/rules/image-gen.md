...
- **Cost runaway from delegated sub-agents.** A sub-agent calling `/image` in a loop is not caught by `.claude/hooks/post-edit-validate.sh` — that validator gates on prod-vs-test classification, not on cost. v1 ships with pre-call estimate as the only signal. If empirical observation shows drift, a future spec adds a per-session call counter.
