  ...
  "required_files": [
    {
      ...
      "contains": [
        ...
        "| Screen | Covers (US-NN) |",
        "| US-NN |",
        "| Screen | Token | Voice | Component | Audit-fix | Specificity |",
        "Deciding signal",
        "Closed-beta partner"
      ]
    },
...
- **The literal `| Screen | Token | Voice | Component | Audit-fix | Specificity |` substring** — proves the design-fidelity 5-dim scoring table exists with all 5 dimension columns. Mirrors step-7's `| Token | Voice | Component | Audit-fix | Specificity |` literal-anchor pattern, prefixed here with `Screen` because the atlas table indexes per-screen (step 7's REPORT table is per-screen too; the literal is structurally identical). The full pipe-delimited fragment only appears as a real markdown table header — bare dimension words appear in prose discussion throughout the atlas, so the original substrings would be silently fakeable.
...
- **Design Fidelity** — markdown table (`| Screen | Token | Voice | Component | Audit-fix | Specificity | Min |`) with one row per screen; Min column = gate indicator (✓ if ≥ 3 across all five dims). Any score < 3 should have been fixed in a pre-emit pass.
