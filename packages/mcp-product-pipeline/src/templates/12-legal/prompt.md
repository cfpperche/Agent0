...
**Output file:** `legal-posture.md` in `docs/product/12-legal/`. Single-artifact — no `extra_files`.
...
- **PRD** — `docs/product/08-prd/prd.md` — § Users / Audience drives the regulation applicability (EU residents → GDPR; California residents → CCPA / CPRA; Brazil residents → LGPD; minors → COPPA / GDPR Art 8; employees → state employment privacy laws); § Goals + § Success Metrics drive the v1 user-volume estimate (drives DPIA trigger — large-scale processing under GDPR Art 35); § User Stories drive what data is collected per flow.
- **System design** — `docs/product/09-system-design/system-design.md` — § Data Model is the authoritative source for what PII is collected; § Integrations names every third-party sub-processor (Stripe, Auth0 / Supabase, OpenAI / Anthropic, Sentry, PostHog, SendGrid / Resend, etc); § Stack drives OSS-component inventory (every npm / cargo / pip dependency in production is a license-compatibility question); § Deployment drives data-residency posture (US-region vs EU-region vs multi-region); § Non-Functional § Security drives the encryption-at-rest / encryption-in-transit + backup-retention commitments.
- **Security** — `docs/product/09-system-design/security.md` — compliance posture (LGPD / GDPR / SOC 2 pre-work named here) drives § Regulated Aspects rows; OWASP threat-model surfaces the breach-notification context; auth model (OAuth scopes, session storage, MFA posture) drives the data-handling row on access controls.
- **Cost estimate** — `docs/product/10-cost-estimate/cost-estimate.md` — § Run Cost line items include DPA fees + legal review allocation + (when applicable) SOC 2 audit fees + outside-counsel retainer; this anchors the timing of when counsel-review checkpoints fire in § Open Decisions.
- **Roadmap** — `docs/product/11-roadmap/roadmap.md` — § Horizon § External coordination names the legal-review trigger week (typically late in Polish + Launch phase); § Open Decisions may carry a legal-related deferred decision (e.g. "EU region deferred to v2 — flip if first EU customer requests data residency").
- **Brand** — `docs/product/05-brand/brand.md` — voice (consumer-friendly conversational vs enterprise-formal) calibrates the terms-of-service tone; a B2B SaaS targeting compliance teams must commit to enterprise-formal terms while a consumer mobile app can adopt plain-language ToS.
- **Architecture JSON** — `docs/product/09-system-design/architecture.json` — quick scan confirms the sub-processor list count + the data-flow shape (does data flow to an LLM at runtime? if yes, AI-specific posture fires).
...
Visibility matters — the escape clause sits at the TOP, NOT the bottom. Mirrors anthill-corporate-counsel + anthill-privacy-dpo + anthill-ip-counsel counsel-disclaimer discipline.
...
3. **Privacy Posture** — the regulation matrix + data flows + retention. The dense section. Format (subsectioned by user-flow when ≥3 distinct data-collection contexts exist — closes step-11's user-flow-shaped discipline):
   ...
   - **Data categories + legal basis + retention table.** Mirrors anthill-privacy-dpo § Step 3 template — one row per data category: `| Data Category | Purpose | Legal Basis | Retention Period | Deletion Mechanism |`. Legal basis values: Consent (Art 6(1)(a)) / Contract (Art 6(1)(b)) / Legal Obligation (Art 6(1)(c)) / Vital Interest / Public Task / Legitimate Interest (Art 6(1)(f)). NOT "Consent for everything" — the anti-pattern anthill-privacy-dpo flags as #1.
   - **Data subject rights.** How users exercise access / deletion / portability / objection / rectification. Posture (in-app self-serve toggle vs email-to-DPO-then-72-hour-SLA). The procedure exists; the actual UI / endpoint lives in the engineering spec (`/sdd new <slug>`).
...
8. **Open Decisions** — the legal-posture decisions the founder hasn't made yet that the document is parked on. Each row carries a deciding signal. Mirrors step-9 / 10 / 11 § Open Decisions discipline. Format:
   ```markdown
   ...
   ```
   2-4 rows is the target. Concern tags use the step-11 allow-list extended with `[counsel-review]` for legal-discipline rows.
...
| **B2B SaaS (the spec 026 default)** | Full ~9-11 KB | Privacy + Data Handling + Sub-Processors + Regulated Aspects (SOC 2 prep) | DPA mandatory for enterprise customers; controller-vs-processor split is the dominant framing. SCCs for EU data flows. § Open Decisions usually carries SOC 2 timing + EU residency decisions. |
...
- **AGPL components in SaaS stacks are the load-bearing OSS risk.** AGPL's network copyleft is triggered by SaaS delivery — any AGPL component requires source disclosure OR a commercial license. Flag at § Licensing § OSS components with `Critical — network copyleft triggered` per anthill-ip-counsel.
...
- **Consent is not the default legal basis.** Anthill-privacy-dpo anti-pattern #1. Evaluate per processing activity — Contract (Art 6(1)(b)) covers most account / billing flows; Legitimate Interest (Art 6(1)(f)) covers most analytics; Consent is the basis for marketing email + tracking cookies only.
...
- **Cap-table modeling, equity grants, board resolutions.** Anthill-corporate-counsel covers those; the MCP step 12 ports the *terms / privacy / IP* surface only. Corporate governance is a separate concern.
...
Step 12 ports three anthill skills: **anthill-corporate-counsel** (terms of service, jurisdiction awareness, entity-type awareness, board-action types), **anthill-privacy-dpo** (regulation matrix GDPR / LGPD / CCPA / PIPEDA / COPPA, controller-vs-processor framing, data-categories table with retention, sub-processor disclosure, DPIA trigger, 72-hour breach notification, matrix mode), and **anthill-ip-counsel** (OSS license compatibility matrix MIT / Apache 2.0 / LGPL / GPL / AGPL with SaaS implications, trade-secret protection, trademark clearance, patent FTO vs patentability, AGPL network-copyleft warning). The MCP port absorbs the load-bearing disciplines from all three (counsel-review checkpoint discipline, regulation matrix shape, sub-processor exhaustiveness, AGPL warning, controller-vs-processor per-flow declaration) and inherits their anti-patterns catalog.

The MCP port diverges from anthill on six points worth naming:

1. **Synthesis posture document, NOT corporate-document drafting.** Anthill ships skills that produce *actual legal documents* — board resolutions with "WHEREAS / NOW, THEREFORE" recitals (corporate-counsel), real DPAs with Article 28 clauses (privacy-dpo), real IP analyses with detailed claim mapping (ip-counsel). The MCP port produces a SYNTHESIS POSTURE document — a founder-facing inventory of "here's where v1 lands on each legal axis; outside counsel writes the actual artifacts". Removes the "founder uses an AI-drafted board resolution unchecked" failure mode that anthill's templates structurally invited. The escape clause at the TOP makes the boundary explicit.

2. **Three-skill unification with conditional-section calibration.** Anthill ships THREE separate skills (corporate / privacy / IP). The MCP port unifies into ONE template with product-class-driven section emphasis: § Privacy fires loud for consumer + B2B SaaS; § Licensing fires loud for dev-tools + open-source; § Regulated Aspects fires only when triggers detect; § AI-Specific fires only when AI-in-stack signal is yes. Removes the "which skill do I invoke?" decision from the founder; the prior-artifact signals drive the routing. Closes the "one-mode template" audit-smell.

3. **Product-class calibration ladder.** Anthill ships ONE template each (corporate / privacy / IP) with no calibration. The MCP port calibrates posture depth by product class — Micro 6 KB, Consumer Mobile 9 KB, B2B SaaS Full 9-11 KB, AI-stack 11-13 KB, Regulated Vertical 12-15 KB. Closes the "undynamic defaults" audit-smell. Classes overlap additively — a B2B SaaS + AI-stack + Healthcare regime expands to ~14 KB with 4 § Regulated Aspects rows.

4. **No magic-number floors.** Anthill-privacy-dpo's matrix-mode trigger is "argument contains --matrix" OR "scope lists more than 5 requirements" — magic 5. The MCP port replaces with signal-extraction (jurisdiction exposure + AI-in-stack signal + product class drive section coverage). Anti-pattern "at least 2 risks per phase" (step-11 calibration) maps to "every applicable regulation has a posture row, NO minimum count" — coverage is driven by trigger, not by floor.

5. **Sub-agent synthesis, NOT live interview.** Anthill skills are single-orchestrator (one agent does identify → determine → draft → validate). The MCP port is `delegable: true` — fully delegable; the sub-agent reads prior artifacts and synthesizes the posture document with NO live parent interview. The 4 signals (product class / jurisdiction / sub-processor stack / AI-in-stack) are mechanically extracted from prior artifacts, NOT live-interviewed. Mirrors step-9 system-design's synthesis posture (in contrast to step-10 / step-11's `delegable: partial` with parent-only interview inputs). Closes the "single-orchestrator" audit-smell — synthesis from prior artifacts is the correct delegation mode for a step that closes a phase gate.

6. **§ Open Decisions with deciding signals + concern tags (mirrors step-9 / 10 / 11).** Anthill skills emit checklists and recommendations without a deciding-signal mechanism. The MCP port replaces with `## Open Decisions` (decisions WITH a deciding signal that closes the deferral). Concern tags inherit the step-11 allow-list extended with `[counsel-review]` for legal-discipline rows.

Anthill's `.anthill/`-namespaced runtime references are stripped — the MCP doesn't have a fork-side compliance state file. Anthill's halt-protocols (`Halting: ...`) translate to the MCP's `product_step_submit` validation error semantics (`{code: "schema-incomplete", missing_or_invalid: [...]}`). Anthill's "WHEREAS / NOW, THEREFORE" recital templates are NOT carried over — the MCP step's escape clause at the TOP redirects the document's purpose from "draft the artifact" to "brief counsel who drafts the artifact". The 72-hour breach notification + DPIA trigger discipline + AGPL warning + controller-vs-processor framing are inherited intact — those are load-bearing.

(No `Calibration revisions applied (YYYY-MM-DD)` sub-section yet — step 12 hasn't been judge-calibrated. Forward-anchor for post-judge edits when the judge phase fires.)
