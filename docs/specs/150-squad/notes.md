...
- **#3 (path-level, not content-level fingerprint) — FIXED in 150.2.** `_fingerprint` is now content-aware: `git status --porcelain -uall` (lists untracked files individually instead of collapsing them to a single `?? dir/` entry — the dogfood's actual manifestation) and each line gains a `\t<git hash-object>` content hash, so an in-place REWRITE of an already-listed path changes the fingerprint line and `guard` catches it (incl. an in-turn forbidden rewrite). The squad's own `.agent0/.runtime-state/` is excluded from the fingerprint — ephemeral bookkeeping, not work product; without the exclusion the now-content-aware fingerprint would see `state.json` churn as a false `aborted_conflict` (also a defensive backstop for finding #4). New regression `10-fingerprint-content-aware.sh` (TDD red→green; an already-listed forbidden file rewritten in-turn → `aborted_policy`). Squad suite 10/10.
...
### 150.3 — resolution (2026-06-04): hardening from the spec-151 squad run

The **first real-feature `/squad` run** (spec 151) surfaced three process findings (the run's payload); the concrete instances were fixed inside 151, and the *generic* class-prevention landed here as 150.3:

- **F1 (vacuous-green gate) — `squad-contract.md` § Author fail-closed gates.** A suite-wrapping gate is only as strong as its coverage; the 151 gate ran a hardcoded `run-all.sh` that never executed the spec's own test. Discipline now documented: gate `test -f <new-test>` + require a **globbing** runner; gate on the artifact not its proxy; "if removing the implementation leaves the gate green, the gate is wrong." (Class-prevention; a `squad.sh`-level auto-check can't know "the spec's test" generically — left to the contract author per rule-of-three.)
- **F2 (peer bends production to a flawed test) — `SKILL.md` pump review bullet.** A green gate is necessary, not sufficient: the orchestrator must read the peer's diff before accepting / `propose-done` — a peer told to "make the test pass" can add a regression to satisfy a wrong test. Inherently judgment → discipline, not mechanism.
- **F3 (brief scope unenforced) — `squad.json.example` default + test 11.** Only `forbidden_paths` is mechanically enforced (the NL brief is a hint). The default contract now forbids `\.agent0/HANDOFF\.md` so a peer can't rewrite the orchestrator-owned handoff mid-build. New regression `11-contract-example-forbids-handoff.sh` (TDD red→green). Squad suite 11/11.
