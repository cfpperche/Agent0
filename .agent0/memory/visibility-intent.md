...
Resolved: spec 011 (runtime-introspect, delivered) realized this intent for the local test/build probe wedge; spec 012 (mcp-recipes) for adoption of external MCPs (Playwright, DBHub, etc.). Future visibility capacities (transcript self-query, DB introspection in pyshrnk, dev-server log forwarding) continue under this framing.
