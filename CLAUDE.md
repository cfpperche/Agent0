...
`Agent` dispatches are gated: `.agent0/hooks/delegation-gate.sh` enforces a 5-field handoff (TASK / CONTEXT / CONSTRAINTS / DELIVERABLE-or-DONE_WHEN), and `.agent0/hooks/delegation-verify.sh` verifies sub-agent work at close (`SubagentStop`, runtime-neutral). See `.claude/rules/delegation.md`.
...
`/vuln-audit` (engine `.agent0/tools/vuln-audit.sh`, osv-scanner) detects known-vulnerable INSTALLED dependencies on demand, stack-aware across whatever ecosystems the project has, runtime-neutral (Claude Code + Codex). The positive replacement for the removed supply-chain gate (spec 112): don't gate install/commit — detect vulnerable locked libs and act. Reports + proposes upgrades; never auto-fixes. Three-bucket coverage report (found/covered/skipped), result status (`clean|findings|unavailable|failed`) decoupled from exit code (default 0; opt-in `--exit-code` for consumer CI). See `.claude/rules/vuln-audit.md`.
...
On an auth-gated URL with no saved state the agent emits `BROWSER_AUTH_REQUIRED: <host>`; the human logs in via a headed Playwright MCP session and the state (`.agent0/.browser-state/<host>.json`) is reused for headless reads. See `.claude/rules/browser-auth.md`.
