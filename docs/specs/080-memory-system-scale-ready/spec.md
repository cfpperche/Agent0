...
| MS-3 | Per-compaction snapshot history (`.claude/.compact-history/<ISO>.md`) | 081 | pending | Quick-win; fixes documented gotcha in compaction-continuity.md |
...
| MS-6 | Runtime-state subsystem README (`.claude/.runtime-state/README.md`) | 081 | pending | Quick-win; pure doc enumerating existing subsystems |
...
- [ ] **Scenario: 081 closure** — **Given** child spec 081 covering MS-3 + MS-6 exists; **When** 081 status flips to `shipped`; **Then** rows MS-3 and MS-6 in the gap matrix above flip to ✓ and `081 status: shipped`.
...
- [ ] **OQ-5** Migration timing for existing 13 Agent0 memories — at MS-2 ship time, append events for each entry manually in one commit, or let them remain "pre-event-sourcing" entries (file exists but no journal record)? Decision: manual append at ship time, single commit titled `chore(080): backfill memory-events.jsonl from existing entries`. Idempotent if the event journal has unique `entry_id` per add.
