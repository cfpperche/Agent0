...
### Live-verify pass 8 (`/home/goat/rshrnk`, 2026-05-12)

0 findings new; 1 ratification. **rshrnk-side first dogfood pass** — deliberately a gap pass (Rust/Cargo.toml is not parsed by the signal table, see `docs/dogfood-plan.md` in rshrnk for the predeclared expectations). rshrnk's scope is intentionally library-only (no axum/warp/rocket; only the `rand` crate as a transitive dep — confirmed by `grep -E '(axum|warp|rocket|actix-web|hyper|tide|poem)' Cargo.toml` returning exit 1 / no matches). With no Rust web framework even hypothetically present, the SessionStart hint was correctly silent — visible empirically by the absence of any `=== mcp-recipes ===` framing in this session's startup blocks, and confirmed by directly invoking `bash .claude/hooks/mcp-recipes-hint.sh` (exit 0, no stdout). Behaves as designed.

**Ratification of the documented Rust-blindness:** `grep -E '(Cargo\.toml|cargo|rust)' .claude/hooks/mcp-recipes-hint.sh` returns ZERO matches — the hint hook does not reference Cargo.toml in any form. The dogfood-plan's prediction ("Cargo.toml is not parsed at all") is empirically correct.

**Two-layer finding shape (rshrnk-specific vs general):**

1. **rshrnk-specific (NOT a spec 012 issue):** rshrnk has no web framework, so the hint silence is doubly correct (no signals AND no parser). No follow-up needed on this fork.
2. **General (carry-over signal for a future signal-table extension):** ANY Rust fork that ADDS a web framework (axum/warp/rocket/actix-web/hyper/tide/poem) would still see silent hint, because the parser doesn't exist. Mirrors the pyshrnk pass-7 cross-spec consideration about Starlette/FastAPI/Flask/Django — Python ASGI frameworks don't trigger the signal table either. **Proposed fix shape (low priority — documentation-first):** extend the signal table to parse `Cargo.toml` `[dependencies]` for axum/warp/rocket/actix-web/hyper/tide/poem → suggest playwright-mcp + chrome-devtools-mcp (mirror of browser-non-Next branch). Same rationale as pyshrnk pass-7's Python-ASGI suggestion: the recipe doc is one click away, friction is low, but a fork developer who hasn't internalised `.claude/rules/mcp-recipes.md` will miss the nudge. Until a real signal shows Rust forks consistently missing the Playwright opportunity (or Python ASGI forks doing the same), v1 keeps it documentation-only.

Empirical method: signal-table-detector is structurally JS-and-DB-focused (the four mature MCPs it tables are Playwright, Chrome DevTools, DBHub, Next-DevTools — none of which are Rust-specific). The hint's silence on a Rust project is the *correct* outcome under the current design, even when web frameworks ARE declared in Cargo.toml. The proposed Rust signal-table extension is therefore a recipe-coverage call (suggesting Playwright for ANY backend that serves HTML, regardless of language), not a Rust-specific spec — same shape as the pyshrnk pass-7 cross-spec note.
