...
### Layer 6 — Sync-harness coverage check

- [x] 11. Run `bash .claude/tools/sync-harness.sh --check --agent0-path "$(pwd)" "$(pwd)"` and confirm it reports `AGENTS.md`, `CLAUDE.md`, and `.claude/rules/runtime-capabilities.md` on the up-to-date managed path (not as `customized` refusals on the upstream itself). Do not create `.claude/harness-sync-baseline.json` in the Agent0 source repo; that baseline is fork-local state written by `--apply` in downstream forks.

## Verification

- [x] 12. Run `bash .claude/tests/runtime-capabilities/run-all.sh` — all eight scenarios pass.
- [x] 13. Run `bash .claude/tests/instruction-drift/run-all.sh` — all existing scenarios still pass (non-regression; the new helper function must not break the existing AGENTS.md baseline check).
- [x] 14. Run `bash .claude/tools/check-instruction-drift.sh --root "$(pwd)" --agent0-path "$(pwd)"` against the real repo state — exit 0; all five new anchor invariants report `ok` alongside the pre-existing checks.
- [x] 15. **Dogfood verification** — open `.claude/rules/runtime-capabilities.md` directly and confirm the MCP recipes row's Claude cell is `native-opt-in` (not plain `native`), the Codex cell carries `convention` or `planned: <slug>`, and the owner-file column lists at least three files. Confirm `## Codex Capability Tiers` does not appear anywhere in `AGENTS.md` (`grep -c '^## Codex Capability Tiers' AGENTS.md` returns `0`). Confirm AGENTS.md and CLAUDE.md managed blocks remain byte-identical (the existing managed-block parity check passes).
...
- [x] 17. Update `docs/specs/093-runtime-capability-registry/spec.md` `**Status:**` from `draft` to `shipped` and check off every `- [ ]` acceptance bullet to `- [x]`. Leave changes ready for Claude review before any commit.
