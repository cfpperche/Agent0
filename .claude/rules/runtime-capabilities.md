...
| MCP recipes | `native-opt-in` | `convention` | `.claude/rules/mcp-recipes.md`; `.mcp.json.example`; `.claude/hooks/mcp-recipes-hint.sh` | Worked-example row: Claude support requires recipe activation and credentials where applicable; Codex follows the recipe manually until a parity spec proves native config support. |
