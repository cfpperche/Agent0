...
### Dogfood pass-1 findings (`/home/goat/rshrnk`, 2026-05-12)

First rshrnk-side dogfood pass — deliberately a gap-finding pass (Rust/cargo is NOT in spec 011's v1 detector pair list; see `docs/dogfood-plan.md` in rshrnk for the predeclared expectations). Exercised every Cargo verb a Rust developer would reach for daily. Five gap-confirming findings, one negative-workaround finding that elevates the discovery from "missing detectors" to "missing detectors AND no mid-session escape hatch", one partial spec-020 ratification. All eight findings cohere into two distinct follow-up specs.

1. **`cargo test` (passing) — no detector match.** Ran `cargo test` against the 4-test passing suite; subsequent Bash invocation `bash .claude/tools/probe.sh last-run` returns `status: no-snapshot`. The `.claude/.runtime-state/` directory contains only `in-flight/<id>.t` marks (from the pre-mark hook, which runs unconditionally on every Bash) — no `last-run.json` was ever written. Skip-not-detect path in `runtime-capture.sh` exits silently. **Severity:** significant — `cargo test` is the primary Rust verifier; under spec 011's current detector list, the entire Rust testing surface is invisible.
2. **`cargo test` (failing, exit 101) — no detector match, skip path runs regardless of event source.** Created `tests/spec020_dogfood.rs` with `assert!(false, ...)`, ran `cargo test --test spec020_dogfood` (exit 101 surfaced to the harness — `<error>Exit code 101</error>` block in the tool result confirms PostToolUseFailure path was taken). Probe still returns `no-snapshot`. **Important corollary:** spec 020's PostToolUseFailure registration is correctly synced in rshrnk's `.claude/settings.json` (`jq '.hooks.PostToolUseFailure'` confirms `runtime-capture.sh` registered on the failure event), but the detector mismatch happens BEFORE the event-specific FAIL-default branch is reached. The gap is in the detector pair list, not in event registration — spec 020's fix is orthogonal but inert when the runner isn't detected at all.
3. **`cargo build` — no detector match.** `cargo build` completes cleanly (`Finished `dev` profile`); probe returns `no-snapshot`. Same shape as #1.
4. **`cargo check` — no detector match.** `cargo check` (fast typecheck-equivalent, the closest cargo verb to `bun tsc --noEmit`) completes cleanly; probe returns `no-snapshot`. Same shape as #1.
5. **`cargo clippy --all-targets -- -D warnings` — no detector match.** Clippy (the Rust lint analog to biome/ruff) completes cleanly; probe returns `no-snapshot`. Same shape as #1.
6. **`CLAUDE_RUNTIME_INTROSPECT_EXTRA_DETECT` is NOT settable mid-session by the agent (NEGATIVE finding).** The rule doc (`.claude/rules/runtime-introspect.md` § Escape hatches) documents the env var as the extension path: `CLAUDE_RUNTIME_INTROSPECT_EXTRA_DETECT="<space-separated keys>"` adds custom runners. Two empirical attempts:
   - Inline-prefix form: `CLAUDE_RUNTIME_INTROSPECT_EXTRA_DETECT="cargo-test cargo-check cargo-clippy cargo-build" cargo test` → probe `no-snapshot`.
   - Export-form: `export CLAUDE_RUNTIME_INTROSPECT_EXTRA_DETECT="..."; cargo test` (within a single Bash tool call) → probe `no-snapshot`.

   Root cause: the hook (`runtime-capture.sh`) reads the env var via shell expansion `${CLAUDE_RUNTIME_INTROSPECT_EXTRA_DETECT:-}` from its own process env. The hook is spawned by the Claude Code harness as a sibling process to the Bash command — it inherits the harness's env (which is the env that the human's shell had when `claude` was launched), NOT the env of any individual Bash tool command. Mutations made inside a Bash tool's child shell (prefix or export) never reach the harness's env table. Confirmed structurally: `jq 'keys' .claude/settings.json` returns only `["hooks"]` — there is no `env` injection key for settings.json that could bridge this gap.

   **Impact:** the rule doc's "extension via env var" is reachable ONLY by the human pre-launch (`export VAR=...; claude`). An agent that discovers mid-session it would benefit from EXTRA_DETECT has no in-band path to enable it. Forks running on undetected stacks (Rust today; whatever the next gap is tomorrow) get zero capture even with the documented escape hatch — the agent can't apply the escape from within the session that surfaces the need.

   **Proposed fix shape:** (a) document the pre-launch constraint prominently in `.claude/rules/runtime-introspect.md` § Escape hatches (one sentence, "must be set in the shell that launches `claude`, not via inline prefix or `export` inside a Bash tool call"); AND/OR (b) provide an in-band injection mechanism — e.g. a `.claude/.runtime-detect-extra` plain-text file the hook reads in addition to the env var, which the agent CAN write to mid-session, with the same `<tool>-<verb>` line-per-entry shape. Option (b) is the materially useful fix; option (a) alone closes the doc gap without closing the capability gap.
7. **Spec 020 + EXTRA_DETECT composition: cannot be empirically validated in this pass.** Phase 3 of the dogfood plan (combine PostToolUseFailure path with extra-detected cargo) collapsed into Phase 1.2 because EXTRA_DETECT doesn't propagate (#6). Without a detector match, the failure-path branch never runs — there's nothing to compose. Spec 020's fix is verified in shrnk dogfood pass-3 (bun-test FAIL case) and pyshrnk dogfood pass-2; its orthogonality to detector source IS the documented design, but the rshrnk pass cannot ratify it for the `extra:` detector branch specifically until #6 has a fix.
8. **Two distinct follow-up specs proposed (both gating graduation):**
   - **`spec 0XX runtime-introspect-rust-detector`** — add the cargo verb pair list to the detector table: `cargo test`, `cargo build`, `cargo check`, `cargo clippy` (with `--all-targets -- -D warnings` shape), `cargo run` (with keyword filter mirroring `bun run` — skip when script-arg is `dev`/non-verifier). Add Rust inference patterns: test runner `test result: ok\. <N> passed` → PASS, `test result: FAILED\. <N> failed` → FAIL; clippy/check `^error\[E[0-9]+\]:` or `^error:` → FAIL, `^warning:` (with `-D warnings` clippy) → FAIL, clean output (no `error`/`warning` keywords, completes via `Finished ...` line) → PASS. The pattern shape is established (sister of Python and Bun branches); the table extension is mechanical once cargo is declared in scope.
   - **`spec 0YY runtime-introspect-extra-detect-injection`** — provide a mid-session-writable path for EXTRA_DETECT. Smallest shape: hook reads from `.claude/.runtime-detect-extra` (if exists) AND from the env var, unioning the two sources. Plain text, one `<tool>-<verb>` per line, agent writes via `Edit`/`Write`, takes effect on the next matched Bash. No settings.json mutation, no harness change. Spec 0XX (Rust detector) reduces the *immediate* need for 0YY by absorbing the cargo case directly, but 0YY remains useful for the next undetected stack (gleam? deno? hatch? bazel?) — and importantly de-risks the harness-extension cadence: forks aren't bottlenecked on upstream Agent0 spec landing to capture novel runners.

Method note on PostToolUseFailure observation (carry-forward from pyshrnk pass-1): the harness surfaces non-zero Bash exits to the tool-result block as `<error>Exit code N</error>` — that's the observable signal that `PostToolUseFailure(Bash)` (rather than `PostToolUse(Bash)`) was the firing event. The hook's actual decision (skip vs capture) is internal; the only externally-visible state delta is `last-run.json`. The combination "Exit code surfaced AND probe still `no-snapshot`" is the unambiguous evidence that the failure-event hook ran but skip-not-detect short-circuited it. Recorded so future fork dogfoods can replicate the observation cheaply.
