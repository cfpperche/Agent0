#!/usr/bin/env bash
# .claude/tests/propagation-advisory/run-all.sh
# Orchestrator for propagation-advise.sh scenarios. Runs every NN-*.sh in lex
# order and prints a summary table. Exits 0 if all pass, 1 if any fail.
#
# Usage:
...
results=""
any_fail=0
tmpout="$(mktemp -t pa-run-all-XXXXXX)"
...
printf '\n=== propagation-advisory scenario results ===\n'
printf '%s\n' "$results"
printf '=============================================\n'
