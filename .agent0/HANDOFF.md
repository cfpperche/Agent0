...
- **SDD-flow backlog from the debate — status after this session:**
  ...
  - ✅ **`sdd close` advisory — done + pushed + synced** (`1b8830b`, spec 179). Built full SDD pipeline; opt-in-via-Closure design (silent on corpus). On Agent0 `origin/main` + the 3 active consumers' `origin/main`.
...
- **All Agent0 + 3-active-consumer work this session is pushed.** Nothing pending to push. Working trees clean across all four repos.
- **Sync the remaining consumers when desired** (deferred by choice; the active 3 are done): ag-antecipa / tmux-sentinel / tese are `--check`-clean (visual-contract.md is a new additive file for them). **codexeng needs a separate, deliberate migration** — it is on the old `.claude/` rule layout; a sync would copy `.agent0/context/rules/*` AND delete old `.claude/rules/*` (structural, broader than this wave). Do not bundle it.
- **Best remaining SDD-flow work:** the **debate tiers / "always decision-grade" contradiction** spec (spans `spec-driven.md` + `.claude/skills/sdd/SKILL.md`). Lower urgency.
- Optionally adopt the `**Verify:**` / `**Closure:**` conventions in future specs (both opt-in).
