...
| 6 | shared shell tools | `.claude/tools/*.sh` (sync-harness, probe, lib, routines, instruction-drift, `codex-local-env`) → `.agent0/tools/` | `move` | Phase 3 — `105-shared-tools-to-agent0` (planned) | Decided 2026-05-28. `.agent0/tools/` already exists (memory tools moved there in the 103 lineage); this row closes the residual split (incl. `codex-local-env.sh`, a Codex-facing tool sitting under `.claude/`). Largest blast radius (~85 refs across tests/rules/docs) → own child spec to avoid a mega-diff |
