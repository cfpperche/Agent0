...
Codex does not automatically load dotenv files. If a recipe uses `env_vars = ["DATABASE_URL"]` or `bearer_token_env_var = "FAL_KEY"`, those names must exist in the environment of the Codex process. For consumer projects, prefer a gitignored `.codex/.env.local` plus `bash .claude/tools/codex-local-env.sh` so credentials are scoped to that one Codex process instead of exported at OS level. For plain direct `codex` startup, export the required env vars before launching Codex. Do not put literal secrets in the template or any tracked file.
...
**Codex CLI activation:** `codex mcp add fal-ai --url https://mcp.fal.ai/mcp --bearer-token-env-var FAL_KEY` writes the global Codex config by default in `codex-cli 0.133.0`. Use project TOML when the fal.ai surface should be scoped to one trusted project. `bearer_token_env_var` lets Codex build the bearer auth header from `FAL_KEY` without committing the token. Do not use literal `bearer_token` for fal.ai HTTP config: `codex-cli 0.133.0` rejects it for `streamable_http`.

**Install:** no package install — HTTP endpoint is hosted by fal.ai. Get `FAL_KEY` from fal.ai dashboard, export it in the shell, or put it in gitignored `.codex/.env.local` and launch through `.claude/tools/codex-local-env.sh`. Alternative one-shot registration for Claude Code: `claude mcp add --transport http fal-ai https://mcp.fal.ai/mcp --header "Authorization: Bearer $FAL_KEY"`.
...
- **Codex does not auto-load `.codex/.env.local`.** Use `bash .claude/tools/codex-local-env.sh` to inject project-local secrets only into the Codex process. For direct `codex` startup, export `FAL_KEY` / `DATABASE_URL` in the shell before launching Codex. Avoid shared OS-level exports when different projects need different values.
