...
CONFIG_PATH="$PROJECT_DIR/.claude/memory.config.json"
...
if [ "$USE_HELPER" -eq 1 ]; then
  ...
  CLAUDE_PROJECT_DIR="$PROJECT_DIR" python3 "$HELPER" project-entries | while IFS=$'\t' read -r slug name description; do
