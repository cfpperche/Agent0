...
- [ ] Every `undecided` row is resolved to `move`, `stays`, or `deferred` before this umbrella closes. A `deferred` row must name its revisit trigger (e.g. "when Codex needs rules") — deferral is a recorded decision, not an open ambiguity.
...
| 3 | session-state (shared by both runtimes) | `.claude/.session-state/` | `undecided` | TBD | reverses 101 OQ-E; needs `probe.sh` boundary update + Claude-regression care |
| 4 | runtime-introspect state | `.claude/.runtime-state/` | `undecided` | TBD | Codex is read-only on it today; producer is Claude hooks |
| 5 | browser-auth state | `.claude/.browser-state/` | `undecided` | TBD | both runtimes via Playwright MCP; credential-class gitignore |
| 6 | shared shell tools | `.claude/tools/*.sh` (sync-harness, probe, lib) | `undecided` | TBD | runtime-neutral, but moving touches every doc that cites `.claude/tools/` |
...
- [ ] Row 3 (session-state): does relocating it to `.agent0/.session-state/` justify the `probe.sh` boundary rewrite + Claude-regression risk, or is it the one shared surface that pragmatically stays? (Decide when Phase 1 lands and the pattern is proven.)
