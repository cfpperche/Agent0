...
- [ ] **Scenario: Codex SessionStart injects `.agent0/HANDOFF.md`**
...
- [ ] **Scenario: Codex SessionStart from subdirectory resolves git root**
...
- [ ] **Scenario: Codex Stop hook nags once on dirty WIP (continue-with-corrective-prompt parity)**
...
- [ ] **Scenario: Codex apply_patch edit is attributed to the session**
...
- [ ] **Scenario: HANDOFF.md edits via Codex propagate to Claude without loss**
...
- [ ] **Scenario: existing Claude SessionStart + Stop behavior preserved post-port**
...
- [ ] **Scenario: synthetic SessionStart fixture validates HANDOFF injection without live Codex**
...
- [ ] **Scenario: synthetic Stop fixture validates nag-once via `stop_hook_active`**
...
- [ ] **Scenario: bounded shared-state contract works for both runtimes**
...
- [ ] `.agent0/hooks/session-start.sh`, `.agent0/hooks/session-stop.sh`, and `.agent0/hooks/session-track-edits.sh` exist, are executable, and source `_memory-hook-lib.sh` for project-dir resolution
- [ ] Zero literal `${CLAUDE_PROJECT_DIR:-$PWD}` patterns in the moved scripts; resolution uses `memory_project_dir`
- [ ] `.claude/settings.json` SessionStart / Stop / PostToolUse entries reference `.agent0/hooks/<name>.sh` (zero references to the old `.claude/hooks/session-*.sh` paths in settings)
- [ ] `.codex/config.toml.example` contains commented `[[hooks.SessionStart]]`, `[[hooks.Stop]]` (omitting `matcher` — Codex's `Stop` matcher is documented as currently unused), and `[[hooks.PostToolUse]]` on `^apply_patch$` blocks paralleling the existing memory/readout blocks; an uncommented copy parses successfully via `python3 -c 'import tomllib; tomllib.loads(...)'`
- [ ] `.claude/rules/runtime-capabilities.md` `session handoff` row updates from `convention` to `native-opt-in` for Codex CLI, with owner files reflecting the new `.agent0/hooks/` paths
- [ ] `.claude/rules/session-handoff.md` § *Asymmetric enforcement* updates to describe the new (nag-once-parity) shape; the `.claude/SESSION.md` 3-layer fallback shrinks to a 2-layer (HANDOFF.md → empty-advisory)
- [ ] Sync-harness `--apply --dry-run` against a fixture consumer dir shows the three `.agent0/hooks/session-*.sh` files propagate
...
- [ ] **OQ-E (plan-time design decision): state-dir physical path — keep `.claude/.session-state/` or migrate to `.agent0/.session-state/`?** The 4-file shared-state contract must live somewhere both runtimes read. Keeping `.claude/.session-state/` preserves Claude regression-compat with zero migration; migrating to `.agent0/.session-state/` is consistent with the `.agent0/hooks/` namespace this spec establishes but needs compat handling for in-flight Claude sessions. Not a debate disagreement (both agents agree it must be decided, neither holds a conflicting position) — owner: `/sdd plan`.
