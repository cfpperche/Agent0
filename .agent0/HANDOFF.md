...
- **⚡ Tachyon (packages/tachyon/) is shipped and evolving via umbrella spec 187.** v1 (spec 186, `a388732`) + eight shipped children: sidebar increment (`fa59007`), **F1 attention detection** (188, `6dbd6c9`), **F12 stable port + idempotent registration** (189, `f41480b`), **F2 crash lifecycle** (190, `6f3a053`), **F3 Bridge auth** (191, `a4756c0`), **F4 pins & notes** (192, `54db674`), **F13 agent CRUD UI** (193, `25345e3`), **F14 sidebar taxonomy** (194, `1f0775a`, closed `55cbe01`).
- **F14 (latest):** kind: agent|terminal (inferred from cmd via KNOWN_AI_CLIS, launcher-aware; explicit override) — grouped sidebar (Agents/Terminals), kind-based attention defaults (supersedes watch heuristic), kind in list_agents. F15 commands (one-shot) BRIEFED in umbrella, decision pending. Suite 115/115 + 14-passing integration. Also fixed: agent terminals now isTransient (no ghost bash tabs on restart, 90c525d).
...
- **Dogfood environment prepared** at `~/tachyon-demo/` (outside the repo): `tachyon.yml` (claude/codex/opencode/gemini/shell/fragil agents, layouts, `restart: on-crash` dummy), `.mcp.json` registered with auth header + stable port 41931, `DOGFOOD.md` walkthrough (F1–F4 + F13/F14 scenarios). F5 from the Agent0 window launches it ("Run Tachyon (demo)" config); user validated F1/F2 dogfoods already; F3 dogfood handed off (one ↻ restart of the claude agent needed post-upgrade for env injection).
- Working tree clean; all Tachyon work committed through `55cbe01`.
