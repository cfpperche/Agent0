...
- **No PHP-aware monorepo walk.** The validator's "first lockfile wins" remains — a consumer project with composer.json AND package.json routes to whichever elif matches first (currently JS first, PHP late). Agent0 does NOT run both stacks' pipelines per edit; a PHP+JS monorepo declares `.agent0/validator.json` (spec 207) for full coverage. The fallback now emits a `multi-stack-advisory:` (spec 210) when it detects PHP alongside another stack but audited only one — so the "PHP in `services/api/` got skipped" case is surfaced, not silent.
...
- **Lockfile precedence vs PHP.** The validator's elif chain is `bun → pnpm → npm → python → go → rust → php`. A monorepo with `bun.lock` at root and `composer.json` in `services/api/` routes to bun (first match wins). The PHP detection only fires when `composer.json` is at root AND no JS/Python/Go/Rust manifest precedes it. This is the documented fallback limitation — surfaced (not silent) by the `multi-stack-advisory:` (spec 210, see `typecheck-advisory.md` § Multi-stack honesty advisory) and resolved by declaring `.agent0/validator.json` with package-scoped commands (spec 207).
