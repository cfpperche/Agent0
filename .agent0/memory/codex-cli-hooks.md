...
**Matcher aliasing (updated 2026-06-09, EMPIRICALLY CONFIRMED — interactive):** Codex lets an `Edit` or `Write` *matcher* fire on `apply_patch` edits. Docs verbatim: *"For file edits through `apply_patch`, `matcher` values can use `apply_patch`, `Edit`, or `Write`; hook input still reports `tool_name: "apply_patch"`."* So at the **registration** layer, a matcher `Edit|Write` (NOT `MultiEdit` — not an alias) DOES match on Codex, improving cross-runtime matcher portability. **But the payload `tool_name` stays `apply_patch`** — so a hook *script* that branches on `tool_name == "Edit"`/`"Write"` still won't match, and affected paths still aren't in a `tool_input.file_path` field; the script discovers them by parsing the `apply_patch` patch text or running `git diff --name-only`.

**Confirmation (2026-06-09):** a TEMP `^Edit$` PreToolUse hook was added to the (already-trusted) Agent0 `.codex/hooks.json`, an **interactive** Codex session was driven (human-in-the-loop) to edit a file via `apply_patch`, and the hook **fired** — the captured payload was `{hook_event_name: "PreToolUse", tool_name: "apply_patch", turn_id, tool_use_id, tool_input, cwd, model, permission_mode, session_id, transcript_path}`. `/hooks` showed the `^Edit$` matcher Active. (The earlier 2026-05-27 snapshot said `Edit|Write|MultiEdit` "will not fire" — correct then; matcher aliasing is the drift.)

**Operational gotcha (worth keeping):** the same probe run under `codex exec` (non-interactive, both project- and user-scope `hooks.json`, `--dangerously-bypass-hook-trust`) fired **no** hook at all — not `^Edit$`, not the `^apply_patch$` control, not a `SessionStart` probe — despite `apply_patch` running. So **`codex exec` does not observably run lifecycle hooks**; validate Codex hook behavior only via an **interactive** session, never headless `codex exec`.
