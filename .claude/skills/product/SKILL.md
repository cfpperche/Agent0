...
1. **Step 15 — Screen atlas** (Sub-agent (a)) — dispatch per § Step 15 brief. Returns `<out>/docs/screen-atlas.md` (atlas index) AND `<out>/app/(app)/layout.tsx` (shared chrome for sitemap primary+admin routes) AND optionally `<out>/app/(marketing)/layout.tsx` (when sitemap has ≥3 marketing routes). **Absorbs the responsibilities of deleted Step 7 (prototype-v2 brand-tuned)** per Decision 8 + 14 — the atlas IS the brand+tokens-applied hi-fi pass; there is no separate intermediate prototype. **Per spec 052, atlas MUST run BEFORE the per-route screen-writers** (they consume the layout files atlas writes; chrome inheritance is implicit via Next.js nested-layout cascade — pages no longer invent their own shell).
2. **Step 15 — Per-route screen writers** (Sub-agent (b)) — dispatch N screen-writers in parallel (cap=5) per § Per-stack screen-writer **AFTER atlas Sub-agent (a) returns** (atlas-first sequence, spec 052). N = full sitemap inventory at standard tier (covers all `required_categories` routes plus legal-mandatory surfaces from Step 09 — consent dialog if applicable from DPIA-trigger). Per-route writers place pages under `app/(app)/<route>/page.tsx` for sitemap categories primary+admin (chrome inherited) and flat `app/<route>/page.tsx` for marketing/auth/booking (chromeless or own group layout).
