...
| --- | --- | --- |
| `.agent0/.runtime-state/` | [`runtime-introspect`](../rules/runtime-introspect.md) | Verifier last-run snapshot (`last-run.json`) + per-`tool_use_id` in-flight start marks under `in-flight/`. Captured by `runtime-pre-mark.sh` / `runtime-capture.sh`; read back by `bash .agent0/tools/probe.sh last-run`. |
