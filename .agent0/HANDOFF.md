...
**Spec 145 `od-vendor-skills-bundles-fate` — OPENED (draft, investigate-then-decide).** Per-subdir audit (incl. `templates/pipeline/`, which my first pass wrongly excluded) corrected the target: most of `vendor/open-design/` IS consumed by Step 02 (`prompts/`+`frames/`+`templates/`) + catalogue-gen (`.cache/`); only the **`skills/` subtree (729 files)** is pipeline-unread. Question narrowed to: keep or drop the `skills/ ← design-templates/` `vendored_paths[]` entry. KEEP-and-document vs DROP is founder-gated; dropping changes `--verify` scope (note: 141 flagged `skills/` as the path that *fails* verify — drop may simplify). Apache attribution + `design-systems/` untouched.
