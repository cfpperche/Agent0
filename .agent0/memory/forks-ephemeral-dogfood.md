...
When Agent0 capacity docs (CLAUDE.md, `.agent0/context/rules/*.md`, sync-harness, MCP READMEs) reference "forks", the term is **forward-looking framing**, not a description of an active ecosystem.
...
- **Capacity propagation paths:** Different from fork-count reality. `.agent0/context/rules/` and `.claude/hooks/` DO ship to any hypothetical fork via sync-harness — they need to be correct + portable even if no fork exists yet. The propagation channel is real even when the consumer base is empty.
...
- NOT a permanent fact. If a third party ever forks Agent0 in earnest (a partner adopts the MCP for production planning, a sibling project depends on `.agent0/context/rules/`), this fact flips — and back-compat posture should flip with it.
...
See also: [[feedback_agent0_changes_ship_via_rules_not_memory]] (user-memory) — the inverse rule, about what DOES ship: `.agent0/context/rules/` propagates, `.agent0/memory/` stays project-local.
