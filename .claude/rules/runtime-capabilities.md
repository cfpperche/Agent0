...
| runtime introspect | `native` | `read-only` | `.claude/hooks/runtime-pre-mark.sh`; `.claude/hooks/runtime-capture.sh`; `.agent0/tools/probe.sh`; `.claude/rules/runtime-introspect.md`; `.agent0/.runtime-state/README.md` | Codex can inspect existing snapshots with shell reads, but Claude hooks produce the snapshots. |
| delegation/subagents | `native` | `native-opt-in` | `.claude/hooks/delegation-gate.sh`; `.agent0/hooks/delegation-start-audit.sh`; `.agent0/hooks/delegation-stop.sh`; `.claude/hooks/post-edit-validate.sh`; `.claude/rules/delegation.md`; `.agent0/delegation-audit.jsonl`; `.codex/config.toml.example` | Claude's `Agent` tool blocks under-specified dispatch (`delegation-gate.sh`, exit 2). Codex has native subagents (`/agent`, `agents.max_depth`) + opt-in `SubagentStart`/`SubagentStop` observability hooks writing the single canonical `.agent0/delegation-audit.jsonl`, but NO pre-dispatch blocking gate — no Codex hook can stop a spawn (spec 106), so the 5-field discipline is convention-only on Codex (orchestrator self-applies it; see `delegation.md` § Codex: convention-only). |
...
| harness sync | `native-opt-in` | `native-opt-in` | `.agent0/tools/sync-harness.sh`; `.claude/rules/harness-sync.md`; `.claude/tests/harness-sync/` | Both runtimes can run the shell tool explicitly; it is never automatic. |
...
_Re-audit pending: the `lifecycle hooks` promotion from `unsupported` to `native` for Codex CLI implies adjacent rows still framed around "Codex CLI cannot intercept" may need similar promotion. `delegation/subagents` was resolved (spec 106 — promoted to `native-opt-in`, observability hooks only). `runtime introspect` remains framed around Claude-only capture and may warrant the same treatment. Track via the next firing of the competitive-harness re-audit routine, or earlier if a downstream spec needs the answer._
