...
- 🔬 Task 6 (chain-semantics spike) — **design-driving part resolved by DOCS** (Claude hooks docs: SubagentStop hooks run in parallel, exit-2 blocks-and-continues, `agent_id` present; `stop_hook_active` confirmed delivered via `delegation-stop.sh:54`). This invalidated the sentinel design → replaced with the counter-contract (see `notes.md`). The **runtime VALUES** (agent_id preserved across a continuation; stop_hook_active=true on the continued stop) are **cold-restart-live-only** — cannot be observed in-session because settings.json hooks load only at cold start.
- ⏳ **Live Claude `SubagentStop` dogfood** (scenarios 1–3, real fire) + **Codex dogfood** (scenario 4) are the only open items — both human-gated on a cold restart (Claude) / Codex `.codex/config.toml` enable + restart. Logic is fully validated via the synthetic test suite; the live fire is the 108/109 "prove the registration fires" step. Codex prompt is in the session handoff.
