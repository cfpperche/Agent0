...
| runtime introspect | `native` | `read-only` | `.claude/hooks/runtime-pre-mark.sh`; `.claude/hooks/runtime-capture.sh`; `.claude/tools/probe.sh`; `.claude/rules/runtime-introspect.md`; `.agent0/.runtime-state/README.md` | Codex can inspect existing snapshots with shell reads, but Claude hooks produce the snapshots. |
