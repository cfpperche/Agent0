...
- [ ] **Scenario: Codex reminders readout fires at SessionStart**
...
- [ ] **Scenario: Codex routines readout fires at SessionStart**
...
- [ ] **Scenario: Codex MCP-recipes hint fires at SessionStart**
...
- [ ] **Scenario: hook-disabled or pending-trust Codex session does not emit readouts**
...
- [ ] **Scenario: existing Claude Code behavior is preserved post-port**
...
- [ ] **Scenario: Codex SessionStart fired from subdirectory launch**
...
- [ ] **Scenario: Codex reminders readout without PyYAML and without yq**
...
- [ ] `.agent0/hooks/reminders-readout.sh`, `.agent0/hooks/routines-readout.sh`, `.agent0/hooks/mcp-recipes-hint.sh` exist, are executable, and are behaviorally-equivalent to the prior `.claude/hooks/` versions
- [ ] Zero literal `$CLAUDE_PROJECT_DIR` references in the moved scripts; project-dir resolution uses the `_memory-hook-lib.sh` precedence chain (`AGENT0_PROJECT_DIR` → `CLAUDE_PROJECT_DIR` → stdin `.cwd` → `git rev-parse --show-toplevel` → `pwd`)
- [ ] `.claude/settings.json` `SessionStart` entries reference `.agent0/hooks/<name>.sh` for the three migrated hooks (zero references to the old `.claude/hooks/<name>.sh` paths in settings.json)
- [ ] `.codex/config.toml.example` contains commented-out `[[hooks.SessionStart]]` blocks for all three hooks, paralleling the existing `memory-decay-readout` block
- [ ] The three `[[hooks.SessionStart]]` blocks in `.codex/config.toml.example`, when uncommented, parse successfully via `python3 -c 'import tomllib; tomllib.loads(...)'` or equivalent — mechanical guard against TOML shape errors
- [ ] `.claude/rules/runtime-capabilities.md` updates: `reminders`, `routines`, and `mcp recipes` rows (or equivalent capability lines) reflect Codex moving from `convention` to `native-opt-in`
- [ ] `.claude/rules/mcp-recipes.md` revises the explicit "only Claude receives the SessionStart stack hint" line
- [ ] Sync-harness manifest carries the three new `.agent0/hooks/` paths (via existing globs or explicit entry); a dry-run on a fixture project shows the files propagate
- [ ] A synthetic `SessionStart` fixture (stdin payload + temp project dir) drives each of the three hooks and produces the expected framed block on stdout — no live Codex required for acceptance
...
- [ ] **Compat shim or hard cutover?** Spec 099 chose Option A (`.claude/hooks/memory-*.sh` shims left for the migration window). With only 3 hooks and a single SessionStart event, both shapes are mechanically cheap. Option B (hard cutover, no shims) would skip the same-day shim-removal follow-up that spec 099 needed and that this session just executed. Owner: this spec, decided at plan phase.
...
- [ ] **Shared library extraction for the three readouts.** Spec 099 introduced `_memory-hook-lib.sh` (shared shell lib for the 4 memory hooks). Should the three readouts share an analogous `_readout-hook-lib.sh` (framed-block emit, common `=== <NAME> ===` headers, runtime-detection), or stay self-contained? Three readouts is below the rule-of-three threshold; lean self-contained unless concrete duplication surfaces during plan. The project-dir resolution helper (per the corresponding static fact above) is the one concrete shared piece — sourcing `_memory-hook-lib.sh` directly for that may be sufficient.
- [ ] **Env var naming: `CLAUDE_*` vs `AGENT0_*` canonical names.** `CLAUDE_SKIP_MCP_RECIPES`, `CLAUDE_MCP_RECIPES_WORKSPACE_DIRS` exist today as Claude-side names. Should `.agent0/hooks/*` documented canonical names move to `AGENT0_*` (e.g. `AGENT0_SKIP_MCP_RECIPES`, `AGENT0_MCP_RECIPES_WORKSPACE_DIRS`) with `CLAUDE_*` kept as aliases, or are `CLAUDE_*` names retained as the public contract? Cross-cutting question — memory hooks have similar surface elsewhere. Resolution likely belongs in a separate spec, but plan.md must name the choice it's adopting locally.
