...
## Removals DO propagate — managed-block merge is wholesale-replace

**Empirically corrected 2026-06-10** (supersedes the prior "append-only / known limitation" claim). The CLAUDE.md managed-block merge is **wholesale-replace, not append-only** — the region between `AGENT0:BEGIN`/`AGENT0:END` is regenerated in full each sync, so deleting a `## ` section from Agent0's CLAUDE.md DOES remove it from a consumer on the next sync (block markers stay intact). Verified live across all 3 consumers when `## Agent0 governance doctrine` was dropped upstream: it auto-removed cleanly in mei-saas/acmeyard/cognixse with zero manual edits, markers intact. The consumer's own CLAUDE.md documents this ("region between AGENT0:BEGIN and AGENT0:END is replaced wholesale on each [sync]"). The earlier append-only claim (citing spec 070's `## PHP / Laravel` persistence) was either pre-this-merge-behavior or never re-verified — treat wholesale-replace as current truth. Rule files are whole-file synced and orphan-deleted, so a relocated/removed rule also auto-deletes downstream. **Net: de-leaking or relocating shipped content needs no manual consumer cleanup — a clean sync carries removals as well as edits.**
