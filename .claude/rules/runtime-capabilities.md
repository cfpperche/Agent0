...
| session handoff | `native` | `convention` | `.agent0/HANDOFF.md`; `.claude/rules/session-handoff.md`; `.claude/hooks/session-start.sh`; `.claude/hooks/session-stop.sh` | Claude injects/enforces by hooks; Codex reads and updates by entrypoint convention. |
...
| lifecycle hooks | `native` | `native` | `.claude/settings.json`; `.claude/hooks/*.sh`; `.agent0/hooks/*.sh`; `.codex/config.toml` (`[hooks]`); `.codex/hooks.json`; `.claude/rules/runtime-introspect.md`; `.claude/rules/session-handoff.md` | Both runtimes expose `PreToolUse` / `PostToolUse` / `SessionStart` / `Stop` / `SubagentStart` / `SubagentStop` / `UserPromptSubmit` / `PreCompact` / `PostCompact` / `PermissionRequest`; payload shape is nearly identical. Claude additionally exposes `PostToolUseFailure` and a richer total surface (~29 events). Codex tool-name catalog differs — first-party edits arrive as `apply_patch` / `Bash`, not `Edit` / `Write` / `MultiEdit`. First-party Agent0 hook ports are tracked per-capacity; see re-audit note below. |
| reminders | `native` | `native-opt-in` | `.claude/rules/reminders.md`; `.agent0/hooks/reminders-readout.sh`; `.claude/settings.json`; `.codex/config.toml.example`; `.claude/reminders.yaml` | Claude emits the SessionStart readout from project settings; Codex receives the same framed readout after project hook activation. |
| routines | `native` | `native-opt-in` | `.claude/rules/routines.md`; `.agent0/hooks/routines-readout.sh`; `.claude/settings.json`; `.codex/config.toml.example`; `.claude/routines/`; `.claude/.routines-state/` | Claude emits pending routine queue readouts from project settings; Codex receives the same queue readout after project hook activation. |
...
| MCP recipes | `native-opt-in` | `native-opt-in` | `.claude/rules/mcp-recipes.md`; `.mcp.json.example`; `.codex/config.toml.example`; `.agent0/hooks/mcp-recipes-hint.sh` | Claude activates via `.mcp.json` and receives the SessionStart hint from project settings; Codex activates via trusted-project `.codex/config.toml` and can opt into the same hint block. Both remain explicit opt-in and credential-gated where applicable. |
...
| customization/sync surfaces | `native` | `convention` | `.claude/tools/sync-harness.sh`; `.claude/rules/harness-sync.md`; `AGENTS.override.md`; nested `AGENTS.md` | Claude-side customization is harness-managed; Codex customization layers through its native instruction-chain convention. |

_Re-audit pending: the `lifecycle hooks` promotion from `unsupported` to `native` for Codex CLI implies adjacent rows still framed around "Codex CLI cannot intercept" (`delegation/subagents`, `runtime introspect`, `session handoff`) may need similar promotion to `native` or `native-opt-in`. Track via the next firing of the competitive-harness re-audit routine, or earlier if a downstream spec needs the answer._
