...
- **spec 206** (superseded spec 155) — UI-producing brief verification in `delegation-verify.sh` no longer parses a `verify-contract` `report.json`; `delegation-verify.sh` just surfaces the validator's advisory family, including `ui-runner-advisory:` (fires when a UI surface changed but the project declares no UI test runner) at `SubagentStop`. UI acceptance is a green project UI test; DONE_WHEN names that test command, not a browser bundle.
