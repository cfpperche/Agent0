...
- [x] 6. **Manual dogfood pass** — exercise the discipline against ≥3 of the spec's behavioral scenarios on the next 1-2 real-work prompts (substantive-with-2-ambiguities, skip-category, exploratory at minimum). The scenarios are by construction not unit-testable; passing means the agent in the next session, after reading the new rule via the cross-reference path, applies it correctly. If the agent forgets, that's the calibration signal for Open Question #2.

  **Result (2026-05-17, session `dfo`)**: 3 distinct scenarios confirmed across 3 fresh-session prompts. Cenário #1 (2+ ambiguities → ask via `AskUserQuestion`) confirmed twice — once with the pronoun-without-antecedent variant (prompt: `refatora isso pra ficar mais limpo`), once with the not-enough-CONTEXT variant (prompt: `adiciona um header de copyright no spec 035` — agent correctly flagged "spec 035" as ambiguous because it references 3 files). Cenário #6 (override bypass) confirmed with `# OVERRIDE: prototipando, improvisa se ambíguo` + vague body — agent acknowledged override, inferred target via `SESSION.md`, produced 3 substantive refactor diagnostics before user interrupted to protect the working tree. Cenários #2 (1-ambiguity → infer), #3 (skip), #4 (exploratory) still unexercised; #2 turned out harder to construct than expected because any reasonably vague prompt tends to surface 2+ ambiguities under strict scoring. Missed-clarifications: 0/3. Discipline holding; v2 hook not warranted by this sample (3-week window still open for additional data per Open Q #2).
