...
_Re-audit pending: the `lifecycle hooks` promotion from `unsupported` to `native` for Codex CLI implies adjacent rows still framed around "Codex CLI cannot intercept" (`delegation/subagents`, `runtime introspect`) may need similar promotion to `native` or `native-opt-in`. Track via the next firing of the competitive-harness re-audit routine, or earlier if a downstream spec needs the answer._
