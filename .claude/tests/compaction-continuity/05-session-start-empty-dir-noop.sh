...
# graceful no-op edge case for compact-history.
#
# Given source=compact AND no .compact-history/ dir (or empty), when SessionStart
# fires, then no compact-history banner is emitted and exit is 0. The normal
# missing-handoff advisory may still be emitted.
...
# NO .compact-history/ dir, NO HANDOFF.md
...
if ! printf '%s' "$output" | grep -q 'handoff-advisory'; then
  printf 'FAIL: branch-A missing normal handoff advisory\n'
  printf 'output: %s\n' "$output"
  exit 1
fi
...
if ! printf '%s' "$output" | grep -q 'handoff-advisory'; then
  printf 'FAIL: branch-B missing normal handoff advisory\n'
  printf 'output: %s\n' "$output"
  exit 1
fi
