...
| context injection / rules | `native` | `native` | `.agent0/context/rules/`; `.agent0/hooks/context-inject.sh`; `.claude/settings.json`; `.codex/hooks.json` | Agent0 does not use Claude Code's native `.claude/rules/` loader as a harness surface. Rule bodies live once under `.agent0/context/rules/`; the shared hydrator emits a startup index and prompt-selected fragments through lifecycle hooks. Claude gets the hook from project settings; Codex gets it from tracked project hooks. Context is behavioral guidance, not hard enforcement. |
...
| session handoff | `native` | `native` | `.agent0/HANDOFF.md`; `.agent0/context/rules/session-handoff.md`; `.agent0/hooks/session-start.sh`; `.agent0/hooks/session-stop.sh`; `.agent0/hooks/session-track-edits.sh`; `.codex/hooks.json` | Claude injects/nags by hooks; Codex receives SessionStart/Stop/apply_patch attribution from tracked project hooks after project/hook trust. Codex Stop uses continue-with-corrective-prompt nag-once parity, not byte-for-byte termination blocking. |
| SDD | `native` | `convention` | `.claude/skills/sdd/SKILL.md`; `.agent0/context/rules/spec-driven.md`; `docs/specs/` | Codex can follow the artifacts manually, but slash-command execution is not Codex-native. |
...
| reminders | `native` | `native` | `.agent0/context/rules/reminders.md`; `.agent0/hooks/reminders-readout.sh`; `.claude/settings.json`; `.codex/hooks.json`; `.agent0/reminders.yaml` | Claude emits the SessionStart readout from project settings; Codex receives the same framed readout from tracked project hooks after project/hook trust. |
| routines | `native` | `native` | `.agent0/context/rules/routines.md`; `.agent0/hooks/routines-readout.sh`; `.claude/settings.json`; `.codex/hooks.json`; `.agent0/routines/`; `.agent0/.routines-state/` | Claude emits pending routine queue readouts from project settings; Codex receives the same queue readout from tracked project hooks after project/hook trust. |
