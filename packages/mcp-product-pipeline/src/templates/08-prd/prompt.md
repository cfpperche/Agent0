...
10. **Audit response** — dedicated `## Audit Response` section that lists every step-4 finding with its routing decision (P0 acceptance / Backlog with `(resolved at step N)` / Backlog with `(deferred — reason)` / new requirement created from this finding). Mirrors steps 6 + 7 `## Audit Response` sections (the Identity-phase audit-trail discipline carries forward unchanged at Specification-phase). Per-finding line shape: `**F-NN** (severity N, heuristic X) → routed to <P0 / P1 / P2 / Backlog> as <ref>; <one-line "why this routing">`. The section is the audit trail; without it, the reader has no way to see whether the PRD consumed step 4 or silently dropped findings.
11. **Open questions** — what's not yet decided. Each has an owner OR a downstream step number where it'll resolve (`step 9 — Q1`, `step 10 — Q2`, etc).
12. **Backlog** — low-priority items, post-v1 candidates, step-4 audit findings tagged `fix_skill_hint: "deferred"`. Single table: `# | Title | Source | Why deferred`. **Backlog is for genuinely-deferred items**, distinct from `## Audit Response` (which carries the audit-trail of routings into P0 / P1 / P2 / etc). A finding routed to Backlog appears in BOTH sections: the Audit Response row documents the routing decision, the Backlog row carries the actionable backlog item.
