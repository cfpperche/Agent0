...
- **Edit / Write / MultiEdit on a shipped path** → scan the edit's new content for 5 leak-pattern kinds: `spec-NNN` (concrete spec numbers), `docs/specs/NNN` (concrete spec dir paths), `anthill` (upstream design lineage), `personal-path` (absolute `/home/<user>/` paths), `memory-pointer` (concrete `.claude/memory/<topic>.md` references).
...
Maintainer-binding surface (5-pattern regex table, shipped-surface set, audit-log promotion policy, deep gotchas about diff-scope semantics, pattern-volume cap, vendor exclusion mechanism) lives in `.claude/memory/propagation-advisory-maintenance.md`.

## Cross-references

- `.claude/memory/propagation-hygiene.md` — the maintainer-binding discipline this hook enforces
- `.claude/memory/propagation-advisory-maintenance.md` — maintainer-binding companion (pattern table, shipped-surface set, deep gotchas)
