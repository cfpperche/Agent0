...
The shared bootstrap block is deliberately actionable-only: it appears when `.agent0/project-core.md.example` exists and `.agent0/project-core.md` is absent, and disappears once the source exists. If it keeps firing after bootstrap, treat that as stale context/a false positive rather than useful noise. Mirror drift after the source exists belongs to `.agent0/tools/project-core-sync.sh --check` / `doctor.sh`, and repair is local: `.agent0/tools/project-core-sync.sh --apply`.
