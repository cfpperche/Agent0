...
The validator's per-language test-pattern table (the heuristic that drives the advisory in the next section) recognises the common conventions out of the box: `*.test.ts|tsx|js|jsx`, `*.spec.*`, and `__tests__/`/`tests/`/`test/` for the JS/TS family; `*_test.py`, `test_*.py`, and `tests/` for Python; `*_test.go` for Go; `tests/` and `*_test.rs`/`*_tests.rs` for Rust. Files matching none of these (and not on the doc/config exclusion list — `*.md`, `*.json`, `*.yml`, `LICENSE`, etc.) are treated as prod. Project conventions outside that table need the env-var override documented in § *Gotchas*.
