...
This template synthesises the archived `anthill-ux-audit` skill with this pipeline's three-mode validation posture. `anthill-ux-audit` contributed the audit spine — Nielsen's 10 heuristics, the WCAG 2.1 AA review, the severity-1–4 findings table, the strengths discipline, the "every finding actionable" rule. Step 4 contributed the `validation_mode` declaration that downstream steps gate on. The deep port keeps both: anthill's `tested` mode was an *expert* audit only — here the expert audit always runs, and the `validation_mode` line declares what *user-level* validation sits on top of it.
