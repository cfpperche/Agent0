...
- [x] 5. Re-read `.claude/skills/product/templates/pipeline/11-cost-estimate/schema.md` § "Required sections" + § "Conditional sections" to copy the canonical 8-required + 3-conditional list verbatim (Overview / Pricing Model / Assumptions / Build Cost / Run Cost / Sensitivity / Risks / Recommendations; conditional: Unit Economics / Projections / Scenarios).
  - **Done 2026-05-23.** Schema actually carries **4 conditional sections** (Unit Economics / Projections / Scenarios / **Break-even**); the plan/task's "3 conditional" count was stale. Followed schema as canonical truth; deviation logged in notes.md.
- [x] 6. In `delegation-briefs.md` § Step 11 → CONSTRAINTS:
...
- [x] 8. In `.claude/skills/product/SKILL.md` § Phase 1 step 3 ("Steps 03 + 04 — parallel fan-out"), rewrite to two serial dispatches: *"Step 03 alone (functional-spec.md). After Step 03 returns, dispatch Step 04 alone — Step 04's brief reads `functional-spec.md`, so the dispatches MUST NOT share a single message."* Keep the rest of the step (quality judge + state update) unchanged.
  - **Done 2026-05-23.**
- [x] 9. In `SKILL.md` § Phase 4 step 1 ("Dispatch Step 15a + 15b + 15c in one message"), rewrite to: *"Dispatch Step 15a + Step 15c in one message (no shared inputs, distinct outputs). After Step 15c returns, dispatch Step 15b — Step 15b's Mood-screen-writer brief in hi-fi mode reads `fixture-spec.md`, so it CANNOT share a message with 15c."* Keep the per-step bullets (15a/15b/15c bodies) — only the dispatch grouping changes.
  - **Done 2026-05-23.** Reframed as "two waves": wave A = 15a + 15c parallel; wave B = 15b after 15c. Header prose + numbered step both reflect the split.
- [x] 10. In `SKILL.md` § Worked example — parallel dispatch in a single message, update the prose:
  ...
  - **Done 2026-05-23.** True-parallelism list now: Step 06+07, Step 11+12, Phase 4 wave A (15a + 15c). Added an explicit "Anti-parallelism" sub-section naming the two CONTEXT-reads-deliverable cases (03→04, 15c→15b). Worked-example code-block reduced to 2 parallel calls for wave A + a second-message block for wave B (the 15b killer-flow fan-out, up to 5).
- [x] 11. In `delegation-briefs.md` § Phase 4 (line ~392), rewrite the paragraph that claims "three sub-agents — parallelizable in ONE message — all inputs are on disk from Phases 1-3, distinct output paths, no FS race": replace with the truthful split — 15a + 15c parallel; 15b serial because Mood-screen-writer hi-fi mode reads `fixture-spec.md`.
  - **Done 2026-05-23.** § Phase 4 prose now describes "two waves" with the same wording as SKILL.md. § Concurrency cap (line 545) also updated to reflect wave A (2 calls) + wave B (5-screen fan-out) — wasn't in the literal task scope but the prose there had the same false claim.
- [x] 12. In `delegation-briefs.md` § Mood-screen-writer (no edit required) — verify line ~474 still explicitly names `fixture-spec.md` as a hi-fi-mode read (it does; cross-check left in place as the load-bearing source of truth for #5).
  - **Done 2026-05-23.** Verified at line 490: "hi-fi mode ALSO reads: ... fixture-spec.md at {{out}}/docs/fixture-spec.md for the mock data this screen renders." No edit needed.
- [x] 13. In `.claude/skills/product/references/state-machine.md` § Phase progression DAG (lines ~83-108):
  ...
  - **Done 2026-05-23.** Both DAG entries rewritten; inline rationale cites spec 076 #5. To keep the task-31 negative grep clean, the rationale text uses "Steps 03 and 04" instead of the literal `03+04` substring.
- [x] 14. Commit as `fix(076): serialize Step 04 after 03 and Step 15b after 15c (#5)`.
  - **Done 2026-05-23 (commit `6013166`).**

### #4 — Phase 4 visual check over HTTP

- [x] 15. Create `.claude/skills/product/scripts/serve-hifi.sh`: best-effort HTTP server launcher. Picks a free port via `python3 -c 'import socket; s=socket.socket(); s.bind(("",0)); print(s.getsockname()[1])'`; backgrounds `python3 -m http.server <port> --bind 127.0.0.1 -d <serve-dir>`; writes `READY <port>` to stdout when bound (polls until the port responds, with a 5s timeout); on `python3` absent or port-bind failure, exits non-zero with `not-available: <reason>` on stderr. Make executable (`chmod +x`).
  - **Done 2026-05-23.** Script binds via bash-native `/dev/tcp` readiness probe (no `nc` / `curl` dependency); signal trap reaps the child http.server on caller's SIGTERM/INT (so `kill %1` cleans up the port without leak). Four `not-available:` failure modes covered: missing serve dir, missing python3, free-port probe failure, 5s bind timeout.
- [x] 16. In `SKILL.md` § Phase 4 step 3 ("Best-effort visual check"), rewrite the loop:
...
- [x] 28. **Scenario #4** — manual: run the helper script against a directory of 2-3 sample HTML files; `curl http://127.0.0.1:<port>/<file>` returns the bytes; kill the server; `ss -ltn` shows no leaked port. `SKILL.md` no longer contains the string `file://` in the Phase 4 section (`grep -nF 'file://' .claude/skills/product/SKILL.md` returns nothing inside Phase 4; matches elsewhere are fine).
  - **Done 2026-05-23.** Both halves verified: (a) smoke test against `/tmp/serve-test/index.html` returned `<html>ok</html>` via curl, `kill %1` released the port cleanly (no `ss -ltn` listener leak); (b) `grep -nF 'file://' .claude/skills/product/SKILL.md` returns zero matches anywhere (not just Phase 4 — went stricter than the criterion).
- [x] 29. **Scenario #2-sections** — `grep -A1 'Required H2 sections' .claude/skills/product/references/delegation-briefs.md | grep -A0 'Step 11'` covers every required section the schema enforces at Layer 1; the substring `SKIP unit economics + sensitivity + scenario analysis` no longer appears in the Step 11 brief.
  - **Done 2026-05-23.** `grep -c 'SKIP unit economics'` returns 0; the brief's "Required H2 sections (Layer 1 enforced — all 8, unconditional)" line names all 8 schema-required H2s verbatim (Overview / Pricing Model / Assumptions / Build Cost / Run Cost / Sensitivity / Risks / Recommendations).
